survivalAnalysis <- function (data) {
	...
	summ <- summary(cobj)
	output <- list(n=summ$n, n_event=summ$nevent, coef=summ$coefficients[[1]], exp_coef=summ$coefficients[[2]], se_ceof=summ$coefficients[[3]], z=summ$coefficients[[4]], pValue=summ$coefficients[[5]], ci_lower=summ$conf.int[[3]], ci_upper=summ$conf.int[[4]], logrank=list(score=summ$sctest[[1]], pValue=summ$sctest[[3]]), plot=list(control=prepare_plot_data(so_control), experiment=prepare_plot_data(so_experiment)))
