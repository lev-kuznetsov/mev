...
@R ("function (dataset, genelist, species, goType, testType, pAdjust, nodeSize) {\n"
    + "if (species == 'human') {\n"
    + "  anno.db = 'org.Hs.eg.db';\n"
    + "  bg = as.vector (read.csv (file = file.path (cwd, '../topgo/domain/src/main/resources/HSAPIENS_ALL_GENES.txt'))[ , 1 ]);\n"
    + "} else if (species == 'mouse') {\n"
    + "  anno.db = 'org.Mm.eg.db';\n"
    + "  bg = as.vector (read.csv (file = file.path (cwd, '../topgo/domain/src/main/resources/MMUSCULUS_ALL_GENES.txt'))[ , 1 ]);\n"
    + "};\n"
