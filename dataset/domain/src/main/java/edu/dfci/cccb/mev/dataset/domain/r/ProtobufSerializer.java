...
public class ProtobufSerializer {
  ...
  public void serialize(Dataset dataset, OutputStream target) throws InvalidDimensionTypeException, InvalidCoordinateException, IOException, MevException{
...
//    FieldDescriptor fieldAttrName = RExp.getDescriptor ().findFieldByNumber (RExp.ATTRNAME_FIELD_NUMBER);    
...
//    assertTrue(result.isInitialized ());
    ...
    if(errors.size() > 0 )
      throw new MevException(String.format("Serialization errors: %s", errors)) {private static final long serialVersionUID = 1L;};     
    result.writeTo (cos);
    cos.flush ();  
