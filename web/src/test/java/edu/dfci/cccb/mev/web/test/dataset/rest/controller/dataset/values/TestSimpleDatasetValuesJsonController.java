...
public class TestSimpleDatasetValuesJsonController {
  ...
  public void test () throws Exception {
    ...
    MvcResult mvcResult = this.mockMvc.perform(get("/dataset/mock_set/data/values").param ("format", "json")
     ...
     String sValues = mvcResult.getResponse ().getContentAsString();
     Value[] values = jsonObjectMapper.readValue(sValues, SimpleValue[].class);
     ...
     for(int irow = 0; irow < mockDataset.dimension (Type.ROW).keys ().size (); irow++){
       for(int icol = 0; icol < numOfCols; icol++){
         ...
         assertThat (values[irow*numOfCols+icol].value(), IsEqual.equalTo (mockDataset.values ().get (row, column))); ;
       }
     }
  }
