...
// Updates hazard function by multiplying each coefficient by its new value
	...
	function HealthvisSurvival() {
		...
		this.init = function(elementId, d3Params) {
			...
			var dimensions = (typeof healthvis != "undefined") ? healthvis.getDimensions(this.w, this.h) : {height: this.h, width: this.w}  
			this.w = dimensions.width;
			this.h = dimensions.height;
			
			this.vis = d3.select(elementId).append('svg:svg').attr('width', this.w)
			.attr('height', this.h + 30).append('svg:g').attr('transform',
					'translate(' + 40 + ',' + 10 + ')');
			...
			// scales
			this.x = d3.scale.linear().domain([ 0, d3Params.daymax ]).range(
					[ 0, this.w ]);
		...
		this.visualize = function() {
			...
			// Add the x-axis.
			this.vis.append('svg:g').attr('class', 'x axis').attr('transform',
					'translate(0,' + (this.h - 30) + ')').call(xAxis);
			
			this.vis.append('text').attr('class', 'x label').attr('x',
					this.w / 2 - 12).attr('y', this.h + 10).text('Time');
			
			// create left yAxis
			var yAxis = d3.svg.axis().scale(this.y).ticks(6).orient('left');
			
			// Add the y-axis to the left
			this.vis.append('svg:g').attr('class', 'y axis').attr('transform',
			'translate(-5,0)').call(yAxis);
			
			this.vis.append('text').attr('class', 'y label').attr('x', -200).attr(
					'y', -30).attr('transform', 'rotate(-90)').text('Survival');
			...
			// Add path layer
			this.vis.selectAll('.line').data(data).enter().append('path').attr(
					'class', 'line').style('stroke', function(d, i) {
						return colors[i];
					}).attr('d', this.line);
			...
			if (!(group_names == "")) {
				...
				var xOffset = 200;
				var xSpacing = 10;
				var legend = this.vis.append('g').attr('class', 'legend').attr('x',
						this.w - xOffset).attr('y', 125).attr('height', 200).attr(
								'width', 200);
				
				legend.selectAll('rect').data(colors).enter().append('rect').attr(
						'x', this.w - xOffset).attr('y', function(d, i) {
							return i * 20;
						}).attr('width', 10).attr('height', 10).style('fill', function(d) {
							return d;
						});
				
				legend.selectAll('text').data(group_names).enter().append('text')
				.attr('x', this.w - (xOffset-xSpacing*2)).attr('y', function(d, i) {
					return i * 20 + 10;
				}).text(function(d) {
					return d;
				});
		...
		this.update = function(newcov) {
			...
			this.vis.selectAll('path.line').data(tmp).transition().duration(1800)
			.delay(100).ease('elastic').attr('width', 0).attr('d',
					this.line);
